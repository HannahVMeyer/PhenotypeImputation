...
correlation_plot <- function (corr, 
    ...
    if (order != "original") {
        ...
        corr <- corr[ord, ord]
        if (length(tl.col) != 1) tl.col <- tl.col[ord]
    }
    ...
    if (any(is.na(corr)) && is.character(na.label)) {
        ...
        if (na.label == "square") {
            symbols(PosNA, add = TRUE, inches = FALSE, 
                    squares = rep(1, nrow(PosNA)), bg = na.label.col, 
                    fg = na.label.col)
        }
    ...
    if (cl.pos != "n") {
        ...
        if (cl.pos == "b") {
            ...
            ylim <- c(n1 - 0.5 - nn * cl.ratio, n1 - 0.5 - nn * 
                          0.02)
            text(x = mean(xlim),
                 y = n1 - 0.5 - (n1 - 0.5) * 0.01,
                 labels=cl.title, cex=cl.title.cex, adj=0.5)
        }
